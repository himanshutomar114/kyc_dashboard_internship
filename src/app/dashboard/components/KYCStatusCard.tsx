...
const KYCStatusCards = () => {
  ...
  if (!data) {
    return <div className="text-center text-red-500 text-sm">No data available</div>;
  }
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
        {(data.statusCards || []).map((status) => {
          ...
          return (
            ...
            >
              <Icon className={`h-5 w-5 mb-1 ${status.color}`} />
              <p className="text-xs text-gray-600">{status.title}</p>
